...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "application/vnd.ms-powerpoint"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for presentations" ON storage.objects FOR SELECT USING (bucket_id = 'presentations');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for presentations" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'presentations');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for presentations" ON storage.objects FOR UPDATE USING (bucket_id = 'presentations');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for presentations" ON storage.objects FOR DELETE USING (bucket_id = 'presentations');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'presentations',
        public: true,
        allowed_mime_types: ["application/pdf", "application/vnd.ms-powerpoint"],
